...
- [x] **T007** `[수동]` — MongoDB Atlas M0 클러스터 생성
  - 관련 요구사항: `FR-005`
  - 상세: 무료 M0 클러스터 생성 후 연결 문자열(`mongodb+srv://...`) 확보. Network Access에서 `0.0.0.0/0`(Render의 고정 IP가 아니므로 전체 허용 불가피 — 포트폴리오 목적상 허용) 등록
  - 완료 기준: 사용자가 직접 수행하고 완료를 확인함(연결 문자열은 T010에서 Render 환경변수로 직접 입력 예정, AI에게 공유하지 않음)

- [x] **T008** `[수동]` — Upstash Redis 인스턴스 생성
  - 관련 요구사항: `FR-005`
  - 상세: 무료 데이터베이스 생성 후 host/port/password 확보(TLS 필수)
  - 완료 기준: 사용자가 직접 수행하고 완료를 확인함(접속 정보는 T010에서 Render 환경변수로 직접 입력 예정, AI에게 공유하지 않음)

- [x] **T009** `[수동]` — Aiven MySQL 서비스 생성
  - 관련 요구사항: `FR-005`
...
- [x] **T013** `[P]` — SC-001 검증
  ...
  - 확인 완료: `verify/ci-trigger` 임시 브랜치에 push하자마자 `gh run list`에서 `test` job이 `in_progress`로 즉시 트리거됨을 확인. 검증 후 브랜치는 로컬/원격 모두 삭제

- [x] **T014** (T012 완료 후) — SC-002 검증
...
- [x] **T017** `[P]` — SC-006 검증
  ...
  - 확인 완료: 추적 파일 전체 검색 결과 유일한 매치는 `.env.example`의 `MYSQL_PASSWORD=class_platform`(001부터 있던 로컬 개발용 더미 값, 실제 자격증명 아님). Aiven/MongoDB/Upstash/Anthropic 실제 키 패턴은 0건. `git log --all -p` 히스토리 검색도 T012에서 별도로 완료(0건)
