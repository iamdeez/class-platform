...
- [x] **T015** — SC-007, SC-008 통합 테스트 (T010 완료 후, 위 태스크들과 병렬 가능) `[P]`
  - 테스트 파일: `statistics/infrastructure/CourseStatisticsRepositoryImplIT.kt`
  - 검증 대상: `SC-007`, `SC-008`
  - 시나리오: 강의 가격 변경 전후로 신청된 두 수강의 매출 합이 각 신청 시점 가격 기준으로 정확한지 확인, 취소된 수강이 매출에서 제외되는지 확인 — 확인 완료 (해당 파일 총 6개 케이스 모두 PASS)
  - **구현 노트**: 이 프로젝트에는 "강의 가격 변경" 유스케이스 자체가 없다(`Course.price`는 불변). SC-007은 `Enrollment.price` 스냅샷이 `course.price`(현재값)와 무관하게 매출에 반영됨을 검증하는 것이 본질이므로, 두 수강을 서로 다른 `price` 값으로 직접 생성해(가격이 바뀐 시점에 각각 신청한 상황을 시뮬레이션) 매출 합이 두 스냅샷의 합과 일치함을 확인했다.

- [x] **T016** `[P]` — SC-009, SC-010 단위 테스트
  - 테스트 파일: `statistics/infrastructure/CourseStatisticsRepositoryImplTest.kt` (또는 비율 계산 로직을 별도 함수로 분리 시 그 단위 테스트)
  - 검증 대상: `SC-009`, `SC-010`
  - 시나리오: 완료율·취소율 계산 산식 검증(정상 케이스 + 분모 0 케이스) — 확인 완료 (4개 케이스 모두 PASS)
  - **구현 노트**: `CourseStatisticsMapper`를 MockK로 대체해 Testcontainers 없이 순수 단위 테스트로 작성했다(원시 카운트 → 비율 변환 로직만 검증 대상이므로 실제 DB 연동은 T010/T015에서 이미 커버).

## 구현 완료 기준

- [x] 모든 태스크 체크박스가 완료 처리되었다.
- [x] `./gradlew test`가 전체 PASSED를 반환한다. — 확인 완료 (`BUILD SUCCESSFUL in 3m 33s`, 실패 0건)
- [x] `git status`에 의도치 않은 파일이 없다. — 확인 완료
